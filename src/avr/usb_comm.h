...
// Flag for a newly received UBS message
extern volatile uint8_t usb_msg_recv;

// Process current USB message, returns 1 on success and 0 on failure
int8_t usb_comm_process_message(void);

// Reset USB communication status to allow new messages
...
//     uint8_t keymap[64];
//     // Free space which can be used by the password algorithm
// //     uint8_t working_space[43]
